...
namespace Online_Learning_Platform.Controllers
{
    ...
    public class LessonsController : Controller
    {
        ...
        public LessonsController(ApplicationDbContext context, IWebHostEnvironment appEnvironment)
        {
            _context = context;
        ...
        public async Task<IActionResult> Create(LessonViewModel LessonViewModel)
        {
            ...
            if (LessonViewModel.Video != null)
            {
                // путь к папке Files
                string path = "/Files/" + LessonViewModel.Video.FileName;
                // сохраняем файл в папку Files в каталоге wwwroot
                using (var fileStream = new FileStream(_appEnvironment.WebRootPath + path, FileMode.Create))
                {
                    await LessonViewModel.Video.CopyToAsync(fileStream);
                }
                lesson.Path = path;
            }
            else
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Edit(int id, [Bind("Id,Name,Description,CourseId,Path")] Lesson lesson)
        {
            if (id != lesson.Id)
            {
                return NotFound();
